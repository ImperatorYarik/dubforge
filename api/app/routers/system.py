...
async def system_status():
    ...
    # Check model load status from Redis keys set by worker
    r = aioredis.from_url(settings.REDIS_URL)
    try:
        whisper_loaded = await r.get("model:whisper:loaded")
        xtts_loaded = await r.get("model:xtts:loaded")
        whisper_loaded = whisper_loaded == b"1"
        xtts_loaded = xtts_loaded == b"1"
    except Exception:
    ...
    # Check worker online + active jobs via Celery inspect
    worker_online = False
    active_jobs = 0
    queued_jobs = 0
    try:
        ...
        if ping:
            ...
            active_jobs = sum(len(v) for v in active.values())
            queued_jobs = sum(len(v) for v in reserved.values())
    except Exception as e:
